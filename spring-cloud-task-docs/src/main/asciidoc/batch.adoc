...
To override this behavior and allow the task to return an exit code of 1 upon a
batch job returning an
https://docs.spring.io/spring-batch/4.0.x/reference/html/step.html#conditionalFlow[ExitStatus]
of "FAILED", set `spring.cloud.task.batch.command.line.runner.enabled` to true.
