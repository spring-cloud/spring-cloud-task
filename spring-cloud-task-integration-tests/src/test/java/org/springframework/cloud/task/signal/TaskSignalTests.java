...
package org.springframework.cloud.task.signal;
...
@SpringBootTest(classes = TaskSignalTests.TaskLauncherConfiguration.class)
public class TaskSignalTests {
	...
	private final static String URL = "maven://io.spring.cloud:timestamp-task:2.2.0.BUILD-SNAPSHOT";
	...
	private final static String TASK_NAME = "TASK_SIGNAL_TEST";
	...
	public void setup() {
		...
		this.properties.put("waitTimeInMillis", "60000");
	...
	public void testWithLocalDeployer() throws Exception {
		String id = launchTask();
		assertThat(waitForDBToBePopulated()).isTrue();

		this.taskLauncher.cancel(id);
		...
		taskExecutions = this.taskExplorer.findAll(PageRequest.of(0, 10));
		assertThat(taskExecutions.iterator().next().getExitCode().intValue())
				.as("return code should be 143").isEqualTo(143);
	}
	...
	private String launchTask() {
		AppDefinition appDefinition = new AppDefinition(TASK_NAME, this.properties);
		MavenProperties properties = new MavenProperties();
		MavenResourceLoader mavenResourceLoader = new MavenResourceLoader(properties);
		AppDeploymentRequest appDeploymentRequest = new AppDeploymentRequest(
				appDefinition, mavenResourceLoader.getResource(URL), this.properties);

		return taskLauncher.launch(appDeploymentRequest);
	}
