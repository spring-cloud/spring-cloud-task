...
Out of the box, the `TaskRepository` will use an in memory database to record the results
of a task.  Obviously this isn't a practical solution for a production environment since
the database is shut down once the task ends.  However, for a quick getting started
...
The lifecycle of a task is considered from before the `*Runner`s are executed to once they
are all complete.  Spring Boot allows an application to use multiple `*Runner`s and Spring
Cloud Task doesn't attempt to impede on this convetion.
