...
=== Configuring your application to use Task Features

In some cases you will want to use beans instantiated by Spring Cloud Task, for example the
https://docs.spring.io/spring-cloud-task/docs/current/apidocs/org/springframework/cloud/task/repository/TaskExplorer.html[TaskExplorer].
In such cases remember that you should resolve those references at the appropriate
time during the Spring Boot lifecycle.  This means you may wish to autowire those
Spring Cloud Task beans where they are needed instead of attempting to resolve
them in your @Configuration classes (because AutoConfiguration bean resolution
occurs after Configuration bean resolution).
