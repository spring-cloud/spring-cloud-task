...
[[nativeCompilation]]
=== Native Compilation
The advantage of Single Step Batch Processing is that it allows you to dynamically select what reader and writer beans to use at runtime when using the JVM.
But when using native compilation the determination of reader and writer must be done at build time instead of runtime.
