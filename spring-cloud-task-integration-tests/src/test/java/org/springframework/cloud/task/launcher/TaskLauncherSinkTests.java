...
//		properties.put("logging.level.org.springframework.cloud.task", "DEBUG");
	...
	private boolean waitForDBToBePopulated() throws Exception {
		...
		for (int waitTime = 0; waitTime <= MAX_WAIT_TIME; waitTime += WAIT_INTERVAL) {
			System.out.println(">> Table exists: " + tableExists() + " execution count = " + taskExplorer.getTaskExecutionCount());
