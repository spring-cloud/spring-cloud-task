...
the same `spring.cloud.task.name` has a task_lock associated with it.
If so this Task instance will
throw a TaskExecutionException prior to making any entries into the datastore.
At the end of the task execution the lock is released.
...
If Spring Cloud Task detects that a lock exists for a specific task name, it will
also check the timestamp associated with this lock.   If the timestamp {plus}
`spring.cloud.task.single-instance-lock-ttl` is before the current time,
it will clear the old lock and create a new one.  The default for
`spring.cloud.task.single-instance-lock-ttl` is `Integer.MAX_VALUE`.  To
configure this feature add the following property on the task:
```
spring.cloud.task.single-instance-lock-ttl=<time in millis>
```
