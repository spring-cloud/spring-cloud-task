...
context, you can disagle the autoconfiguration by using standard Spring Boot mechanisms.
...
NOTE: You can find a sample remote partition application can be found in the samples module
...
following dependencies for the Spring Cloud Cloud Foundry Deployer:
+
[source,xml]
----
<dependency>
...
This functionality usee a new `CommandLineRunner` that replaces the one provided by Spring
